...
def cut_by_geojson(input_file, output_file, shape_geojson):
    ...
    with open(shape_geojson, 'r') as j:
        contents = gj.loads(j.read())
    # Get coords for bounding box
    x, y = zip(*gj.utils.coords(contents))
    min_x, max_x, min_y, max_y = min(x), max(x), min(y), max(y)
    
    # Open original data as read only
    dataset = gdal.Open(input_file, gdal.GA_ReadOnly)
    #print ("proj:", dataset.GetProjection ())
    #print ("GeoTrans: ", dataset.GetGeoTransform ())
    ...
    #print ("CoordTrans:", wgs84_to_image_trasformation)                                                           
    #print(min_x)
...
cut_by_geojson("D:/KM003320MI_010_MUL_L1T.tif", "D:/test.tif" , "D:/qqq.geojson")